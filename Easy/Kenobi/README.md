...
- Exploiting **ProFTPD 1.3.5** using the `mod_copy` functionality
...
## Understanding the `mod_copy` Vulnerability

One of the vulnerabilities associated with this version involves the ProFTPD `mod_copy` module.
...
I copied the contents of the `id_rsa` file and created the private key on my own machine with the appropriate permissions.
...
The room demonstrated how the `mod_copy` functionality in a vulnerable ProFTPD version could be abused to copy files.
